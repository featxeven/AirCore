...
public final class ConfigManager {
    ...
    public boolean groupFormatEnabled() {
        return config.getBoolean("chat.group-format.enabled", false)
                && config.getConfigurationSection("chat.group-format.formats") != null;
    }
        ...
        if (group != null && !group.isBlank()) {
            for (String key : sec.getKeys(false)) {
                if (key.equalsIgnoreCase(group)) {
                    String format = sec.getString(key);
                    if (format != null && !format.isBlank()) return format;
                }
            }
        }

        return sec.getString("_DEFAULT_");
    }
    ...
    public boolean mentionAllowSelf() { return b("chat.chat-mentions.allow-self-mention", false); }
    ...
    public boolean economyAllowSelfPay() { return b("economy.allow-self-pay", false); }
