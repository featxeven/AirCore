...
public final class LoomCommand implements TabExecutor {

    private final AirCore plugin;

    public LoomCommand(AirCore plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender,
        ...
        if (!player.hasPermission("aircore.command.loom")) {
            MessageUtil.send(player, "errors.no-permission",
                    Map.of("permission", "aircore.command.loom"));
            return true;
        }

        if (plugin.config().errorOnExcessArgs() && args.length > 0) {
            MessageUtil.send(player, "errors.too-many-arguments",
                    Map.of("usage", plugin.config().getUsage("loom", label)));
            return true;
        }
