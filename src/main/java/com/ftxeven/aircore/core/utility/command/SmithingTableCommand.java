...
public final class SmithingTableCommand implements TabExecutor {

    private final AirCore plugin;

    public SmithingTableCommand(AirCore plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(@NotNull CommandSender sender,
        ...
        if (!player.hasPermission("aircore.command.smithingtable")) {
            MessageUtil.send(player, "errors.no-permission",
                    Map.of("permission", "aircore.command.smithingtable"));
            return true;
        }

        if (plugin.config().errorOnExcessArgs() && args.length > 0) {
            MessageUtil.send(player, "errors.too-many-arguments",
                    Map.of("usage", plugin.config().getUsage("smithingtable", label)));
            return true;
        }
