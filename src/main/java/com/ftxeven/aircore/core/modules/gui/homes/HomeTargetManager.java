...
public final class HomeTargetManager implements GuiManager.CustomGuiManager {
    ...
    public Inventory build(Player viewer, Map<String, String> placeholders) {
        ...
        try {
            ...
            HomeSlotMapper.fillHomeInventory(inv, definition, viewer, page, maxPages, homesList, homeSlots, ph, targetLimit, true);
            return inv;
        } finally {
    ...
    public void handleClick(InventoryClickEvent event, Player viewer) {
        ...
        if (genericItem != null) {
            handleGenericItem(genericItem, viewer, event.getClick(), holder.targetName());
        }
    }
    ...
    private void handleHomeClick(String name, Player viewer, ClickType click, HomeTargetHolder holder) {
        ...
        if (applyConfirm != null) {
            ...
            if (match != null) {
                ...
                if (match.equals("delete") && !viewer.hasPermission("aircore.command.delhome.others")) {
                    return;
                }
                ...
                if (matchSec != null) {
                    ...
                    if (!actions.isEmpty()) {
                        itemAction.executeAll(actions, viewer, Map.of(
                                "name", name,
                                "target", holder.targetName(),
                                "player", viewer.getName()
                        ));
                    }
                }
                confirmManager.open(viewer, holder.targetName(), name, match, String.valueOf(holder.page()), holder.getCurrentSort(), holder.getCurrentFilter());
                return;
            }
        }

        handleAction(template, viewer, click, Map.of("name", name, "target", holder.targetName()));
    }
    ...
    public void refresh(Inventory inv, Player viewer, Map<String, String> placeholders) {
        ...
        try {
            ...
            HomeSlotMapper.fillHomeInventory(inv, definition, viewer, page, maxPages, homesList, homeSlots, ph, targetLimit, false);
        } finally {
            CONSTRUCTION_CONTEXT.remove();
        }
    }
