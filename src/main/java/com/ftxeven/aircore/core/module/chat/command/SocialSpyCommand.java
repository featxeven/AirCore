...
public final class SocialSpyCommand implements TabExecutor {
    ...
    private static final String PERMISSION = "aircore.command.socialspy";
    private static final String PERM_OTHERS = "aircore.command.socialspy.others";
        ...
        if (sender instanceof Player p) {
            if (uuid.equals(p.getUniqueId())) {
                ...
                if (resolved.isOnline() && resolved.getPlayer() != null) {
                    MessageUtil.send(resolved.getPlayer(), newState ? "chat.socialspy.enabled-by" : "chat.socialspy.disabled-by",
                            Map.of("player", senderName));
                }
            }
        } else {
            sender.sendMessage("SocialSpy status for " + realName + " -> " + (newState ? "enabled" : "disabled"));
    ...
    private void sendError(Player player, String label, boolean hasOthers) {
        String usage = plugin.commandConfig().getUsage("socialspy", hasOthers ? "others" : null, label);
