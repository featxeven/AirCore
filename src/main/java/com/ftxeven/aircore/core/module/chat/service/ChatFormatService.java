...
public final class ChatFormatService {
    ...
    public Component formatFull(Player player, String rawMessage) {
        ...
        Component full = base.replaceText(b -> b.matchLiteral("%message%").replacement(messageComponent));
        return applyFeatures(player, full);
    }

    public Component processMessageOnly(Player player, String rawMessage) {
        String sanitized = sanitizeForChat(player, rawMessage);
        Component msg = MessageUtil.mini(player, sanitized, Map.of(), false);
        return applyFeatures(player, msg);
    }
    ...
    public String sanitizeForChat(Player player, String raw) {
        ...
        while (m.find()) {
            ...
            boolean allowed = HEX_PATTERN.matcher(lower).matches() ? allColors :
                    COLORS.contains(effectiveTag) ? (allColors || player.hasPermission("aircore.chat.color." + effectiveTag)) :
                            FORMATS.contains(effectiveTag) && (allFormats || player.hasPermission("aircore.chat.format." + effectiveTag));

            if (allowed) sb.append('<').append(content).append('>');
            else sb.append('\\').append('<').append(content).append('>');
            last = m.end();
        }
