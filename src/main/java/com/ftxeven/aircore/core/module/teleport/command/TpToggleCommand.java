...
public final class TpToggleCommand implements TabExecutor {
    ...
    private static final String PERMISSION = "aircore.command.tptoggle";
    private static final String PERM_OTHERS = "aircore.command.tptoggle.others";
        ...
        if (sender instanceof Player p) {
            if (uuid.equals(p.getUniqueId())) {
                ...
                if (resolved.isOnline() && resolved.getPlayer() != null) {
                    MessageUtil.send(resolved.getPlayer(), newState ? "teleport.toggles.enabled-by" : "teleport.toggles.disabled-by",
                            Map.of("player", senderName));
                }
            }
        } else {
            sender.sendMessage("Teleport toggle for " + realName + " -> " + (newState ? "enabled" : "disabled"));
    ...
    private void sendError(Player player, String label, boolean hasOthers) {
        String usage = plugin.commandConfig().getUsage("tptoggle", hasOthers ? "others" : null, label);
