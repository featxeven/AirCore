...
public final class MsgToggleCommand implements TabExecutor {
    ...
    private static final String PERMISSION = "aircore.command.msgtoggle";
    private static final String PERM_OTHERS = "aircore.command.msgtoggle.others";
        ...
        if (sender instanceof Player p) {
            if (uuid.equals(p.getUniqueId())) {
                ...
                if (resolved.isOnline() && resolved.getPlayer() != null) {
                    MessageUtil.send(resolved.getPlayer(), newState ? "chat.toggles.messages.enabled-by" : "chat.toggles.messages.disabled-by",
                            Map.of("player", senderName));
                }
            }
        } else {
            sender.sendMessage("Msgtoggle status for " + realName + " -> " + (newState ? "enabled" : "disabled"));
    ...
    private void sendError(Player player, String label, boolean hasOthers) {
        String usage = plugin.commandConfig().getUsage("msgtoggle", hasOthers ? "others" : null, label);
