...
public final class GodCommand extends BaseCommand {
    ...
    private void apply(CommandSender sender, Scope.Single target, String[] args) {
        boolean enabled = !target.profile().godMode();
        runOnOwner(target, online -> {
            if (enabled && online.isPresent()
                    && blockedByWorldRestriction(sender, online.get(), target.self(), MainConfig.RestrictedFeature.GOD)) {
                return;
            }
            services().players().updateGodMode(target.uuid(), enabled);
            online.ifPresent(player -> player.setInvulnerable(enabled));
            completeCooldown(sender, args);

            String suffix = enabled ? "enabled" : "disabled";
            announce(sender, target.uuid(), target.self(),
                    "utilities.god." + suffix, "utilities.god." + suffix + "-for", "utilities.god." + suffix + "-by", Map.of());
        });
    }
}