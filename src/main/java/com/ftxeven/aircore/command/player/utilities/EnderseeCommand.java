...
public final class EnderseeCommand extends BaseCommand {
    ...
    public EnderseeCommand(Context ctx, PluginGuiManager guis) {
        super(ctx, KEY);
        this.guis = guis;
    }
    ...
    public void execute(CommandSender sender, String label, String subLabel, String[] args) {
        ...
        if (resolver().matchesSelf(viewer, typed)) {
            messenger().send(sender, configs().lang().get("utilities.inventory-view.endersee.errors.self"));
            return;
        }
        ...
        resolveTarget(sender, typed, target -> {
            ...
            guis.openLiveInventory(viewer, target.uuid(), InventoryKind.ENDER, canModify, guiId.orElse(null));
        });
    }
}